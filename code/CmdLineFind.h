...
class CmdLineFind
{
  public:
   ...
   const double find( const std::string& tag, const double deflt, const std::string help = "no help" )
   {
      double result = deflt;
      std::vector<size_t> locate = find( tag );
      if( !locate.empty() && locate[0] < items.size()-1 )
   ...
   const Vector find( const std::string& tag, const Vector deflt, const std::string help = "no help" )
   {
	 ...
	 {
            result =  Vector(   atof( items[locate[i]+1].c_str() ),   atof( items[locate[i]+2].c_str() ),   atof( items[locate[i]+3].c_str() )  ) ;
	 }
	 else
	 {
	    result =  deflt;
